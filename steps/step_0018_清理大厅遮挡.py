...
from config import LOBBY_CLEAN_STABLE_HITS, LOBBY_CLEAR_MAX_ROUNDS, LOBBY_CLEAR_RECHECK_WAIT_SECONDS
...
def clear_lobby_blockers(serial: str) -> None:
    ...
    for round_index in range(1, LOBBY_CLEAR_MAX_ROUNDS + 1):
        ...
        if stable_hits >= LOBBY_CLEAN_STABLE_HITS:
            return
...
def handle_lobby_round(serial: str, round_index: int, stable_hits: int) -> int:
    ...
    if stable_hits:
        wait_seconds(LOBBY_CLEAR_RECHECK_WAIT_SECONDS)
        return stable_hits
...
def log_clean_lobby_hit(serial: str, stable_hits: int) -> int:
    log_info(f"[{serial}] 干净大厅稳定命中 {stable_hits}/{LOBBY_CLEAN_STABLE_HITS}")
    return stable_hits
