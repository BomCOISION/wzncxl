...
def run_bless_rounds(serial: str, candidate_land_indexes: list[int]) -> int:
    ...
    while blessed_count < BLESS_FRIENDS_TARGET_COUNT and candidate_land_indexes:
        ...
        next_bless_number = blessed_count + 1
        if bless_configured_land(serial, land_index, next_bless_number):
...
def bless_configured_land(serial: str, land_index: int, bless_number: int) -> bool:
    ...
    return bless_current_land_and_return_home(serial, land_index, bless_number)
...
def bless_current_land_and_return_home(serial: str, land_index: int, bless_number: int) -> bool:
    ...
    wait_after_wish(bless_number)
    tap_home_button(serial)
    wait_seconds(10)
    return True


def wait_after_wish(bless_number: int) -> None:
    wait_seconds(get_wish_wait_seconds(bless_number))


def get_wish_wait_seconds(bless_number: int) -> float:
    if is_every_tenth_bless(bless_number):
        return 0.544 + 0.05
    return 0.544


def is_every_tenth_bless(bless_number: int) -> bool:
    return bless_number > 0 and bless_number % 10 == 0
